...
def test_get_optimized_kernel():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.get_optimized_kernel(
            X, Y, prior_mu, "se", str_optimizer_method=1
        )
    with pytest.raises(AssertionError) as error:
        package_target.get_optimized_kernel(
            X, Y, prior_mu, "se", str_modelselection_method=1
        )
    with pytest.raises(AssertionError) as error:
    ...
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se"
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
    ...
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "matern32"
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "matern52"
    )
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", str_optimizer_method="BFGS"
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", str_optimizer_method="L-BFGS-B"
    )
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", str_optimizer_method="Nelder-Mead", debug=True
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", str_optimizer_method="SLSQP", debug=True
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", str_optimizer_method="SLSQP-Bounded", debug=True
    )
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", str_modelselection_method="loocv"
    )
    print(hyps)
    ...
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X_set, Y, prior_mu, "set_se"
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X_set, Y, prior_mu, "set_se", str_optimizer_method="L-BFGS-B"
    )
    print(hyps)
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X_set, Y, prior_mu, "set_se", str_modelselection_method="loocv"
    )
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(
        X, Y, prior_mu, "se", use_ard=False
    )
    print(hyps)
