...
def test_sample_functions():
    ...
    functions = package_target.sample_functions(
        np.inf, mu, Sigma, num_samples=num_samples
    )
    assert functions.shape[1] == num_points
    assert functions.shape[0] == num_samples
...
def test_predict_with_cov():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, Y, X_test, cov_X_X, inv_cov_X_X, hyps, str_cov="se", prior_mu="abc"
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, Y, X_test, cov_X_X, inv_cov_X_X, hyps, str_cov=1, prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, Y, X_test, cov_X_X, inv_cov_X_X, 1, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, Y, X_test, cov_X_X, 1, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, Y, X_test, 1, inv_cov_X_X, hyps, str_cov="se", prior_mu=prior_mu
        )

    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, Y, 1, cov_X_X, inv_cov_X_X, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            X, 1, X_test, cov_X_X, inv_cov_X_X, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            1, Y, X_test, cov_X_X, inv_cov_X_X, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_cov(
            np.random.randn(num_X, 1),
            Y,
            X_test,
            cov_X_X,
            inv_cov_X_X,
    ...
    nu_test, mu_test, sigma_test, Sigma_test = package_target.predict_with_cov(
        X, Y, X_test, cov_X_X, inv_cov_X_X, hyps, str_cov="se", prior_mu=prior_mu
    )
    print(nu_test)
    print(mu_test)
    print(sigma_test)
...
def test_predict_with_hyps():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            X, Y, X_test, hyps, str_cov="se", prior_mu="abc"
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            X, Y, X_test, hyps, str_cov=1, prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            X, Y, X_test, 1, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(X, Y, 1, hyps, str_cov="se", prior_mu=prior_mu)
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            X, 1, X_test, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            1, Y, X_test, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            np.random.randn(num_X, 1), Y, X_test, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            np.random.randn(10, dim_X), Y, X_test, hyps, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_hyps(
            X, np.random.randn(10, 1), X_test, hyps, str_cov="se", prior_mu=prior_mu
        )

    nu_test, mu_test, sigma_test, Sigma_test = package_target.predict_with_hyps(
        X, Y, X_test, hyps, str_cov="se", prior_mu=prior_mu
    )
    print(nu_test)
    print(mu_test)
    print(sigma_test)
...
def test_predict_with_optimized_hyps():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            X, Y, X_test, str_cov="se", prior_mu="abc"
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            X, Y, X_test, str_cov=1, prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            X, Y, 1, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            X, 1, X_test, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            1, Y, X_test, str_cov="se", prior_mu=prior_mu
        )

    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            np.random.randn(num_X, 1), Y, X_test, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            np.random.randn(10, dim_X), Y, X_test, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
        package_target.predict_with_optimized_hyps(
            X, np.random.randn(10, 1), X_test, str_cov="se", prior_mu=prior_mu
        )
    with pytest.raises(AssertionError) as error:
    ...
    nu_test, mu_test, sigma_test, Sigma_test = (
        package_target.predict_with_optimized_hyps(X, Y, X_test, debug=True)
    )
    print(nu_test)
    print(mu_test)
    print(sigma_test)
