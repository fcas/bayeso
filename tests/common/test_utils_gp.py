...
def test_validate_common_args():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            1, Y_train, X_test, str_cov, prior_mu, debug
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, 1, X_test, str_cov, prior_mu, debug
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, Y_train, 1, str_cov, prior_mu, debug
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, Y_train, X_test, 1, prior_mu, debug
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(X_train, Y_train, X_test, str_cov, 1, debug)
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, Y_train, X_test, str_cov, prior_mu, 1
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, np.zeros(10), X_test, str_cov, prior_mu, debug
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, np.zeros((3, 1)), X_test, str_cov, prior_mu, debug
        )
    with pytest.raises(AssertionError) as error:
        package_target.validate_common_args(
            X_train, Y_train, np.zeros((3, 2)), str_cov, prior_mu, debug
        )
