...
def test_split():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.split(
            node, depth_max, size_min_leaf, num_features, split_random_location, "abc"
        )
    with pytest.raises(AssertionError) as error:
        package_target.split(
            node, depth_max, size_min_leaf, num_features, split_random_location, 1.0
        )
    with pytest.raises(AssertionError) as error:
        package_target.split(node, depth_max, size_min_leaf, num_features, "abc", 1)
    with pytest.raises(AssertionError) as error:
        package_target.split(
            node, depth_max, size_min_leaf, "abc", split_random_location, 1
        )
    with pytest.raises(AssertionError) as error:
        package_target.split(
            node, depth_max, "abc", num_features, split_random_location, 1
        )
    with pytest.raises(AssertionError) as error:
        package_target.split(
            node, "abc", size_min_leaf, num_features, split_random_location, 1
        )
    with pytest.raises(AssertionError) as error:
        package_target.split(
            X, depth_max, size_min_leaf, num_features, split_random_location, 1
        )
    with pytest.raises(AssertionError) as error:
        package_target.split(
            "abc", depth_max, size_min_leaf, num_features, split_random_location, 1
        )

    package_target.split(
        node, depth_max, size_min_leaf, num_features, split_random_location, 1
    )
...
def test__predict_by_trees():
    ...
    package_target.split(
        node_1, depth_max, size_min_leaf, num_features, split_random_location, 1
    )

    node_2 = package_target._split(X, Y, num_features, split_random_location)
    package_target.split(
        node_2, depth_max, size_min_leaf, num_features, split_random_location, 1
    )

    node_3 = package_target._split(X, Y, num_features, split_random_location)
    package_target.split(
        node_3, depth_max, size_min_leaf, num_features, split_random_location, 1
    )
...
def test_compute_sigma():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.compute_sigma(
            means_leaf, np.array([[1.0], [2.0], [1.0]]), min_sigma=min_sigma
        )
    with pytest.raises(AssertionError) as error:
        package_target.compute_sigma("abc", stds_leaf, min_sigma=min_sigma)
    with pytest.raises(AssertionError) as error:
        package_target.compute_sigma(
            np.array([[1.0], [2.0], [1.0]]), stds_leaf, min_sigma=min_sigma
        )
