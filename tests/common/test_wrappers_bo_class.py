...
def test_load_bayesian_optimization():
    ...
    model_bo = package_target.BayesianOptimization(
        range_2, fun_target, num_iter, debug=True
    )

    for str_acq in constants.ALLOWED_BO_ACQ:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_acq=str_acq
        )
        model_bo.str_acq == str_acq

    for str_cov in constants.ALLOWED_COV:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_cov=str_cov
        )
    ...
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_surrogate=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_cov=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_acq=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, normalize_Y=1
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, use_ard=1
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, prior_mu="abc"
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_initial_method_bo=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_sampling_method_ao=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_optimizer_method_gp=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_optimizer_method_bo=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_mlm_method=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_modelselection_method=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, num_samples_ao="abc"
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, str_exp=123
        )
    with pytest.raises(AssertionError) as error:
        model_bo = package_target.BayesianOptimization(
            range_1, fun_target, num_iter, debug=123
        )
...
def test_optimize_single_iteration():
    ...
    model_bo = package_target.BayesianOptimization(
        range_, fun_target, num_iter, str_exp="test", debug=True
    )
...
def test_optimize_with_all_initial_information():
    ...
    model_bo = package_target.BayesianOptimization(
        range_, fun_target, num_iter, str_exp=None, debug=True
    )
    ...
    X_, Y_, time_all_, time_surrogate_, time_acq_ = (
        model_bo.optimize_with_all_initial_information(X, Y)
    )
...
def test_optimize_with_initial_inputs():
    ...
    model_bo = package_target.BayesianOptimization(
        range_, fun_target, num_iter, debug=True
    )
    ...
    X_, Y_, time_all_, time_surrogate_, time_acq_ = (
        model_bo.optimize_with_initial_inputs(X)
    )
