...
def test_get_range_hyps():
    ...
    cur_range = package_target.get_range_hyps(
        "se", 2, use_ard=False, fix_noise=False, use_gp=False
    )
    print(type(cur_range))
    print(cur_range)
    assert isinstance(cur_range, list)
    ...
    cur_range = package_target.get_range_hyps(
        "se", 2, use_ard=True, fix_noise=False, use_gp=True
    )
    print(type(cur_range))
    print(cur_range)
    assert isinstance(cur_range, list)
    ...
    cur_range = package_target.get_range_hyps(
        "se", 2, use_ard=False, fix_noise=True, use_gp=True
    )
    print(type(cur_range))
    print(cur_range)
    assert isinstance(cur_range, list)
...
def test_convert_hyps():
    ...
    converted_hyps = package_target.convert_hyps(
        "se", cur_hyps, fix_noise=False, use_gp=False
    )
    ...
    converted_hyps = package_target.convert_hyps(
        "se", cur_hyps, fix_noise=True, use_gp=False
    )
    assert len(converted_hyps.shape) == 1
    assert converted_hyps.shape[0] == 4
...
def test_restore_hyps():
    ...
    restored_hyps = package_target.restore_hyps(
        "se", cur_hyps, fix_noise=False, use_gp=False
    )
    assert restored_hyps["noise"] == cur_hyps[0]
...
def test_validate_hyps_arr():
    ...
    with pytest.raises(AssertionError) as error:
        _, is_valid = package_target.validate_hyps_arr(
            cur_hyps, str_cov, num_dim, use_gp="abc"
        )
