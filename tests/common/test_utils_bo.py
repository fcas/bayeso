...
def test_get_best_acquisition_by_evaluation():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.get_best_acquisition_by_evaluation(
            np.arange(-5, 5), fun_objective
        )

    best_initial = package_target.get_best_acquisition_by_evaluation(
        arr_initials, fun_objective
    )
...
def test_check_hyps_convergence():
    ...
    assert package_target.check_hyps_convergence(
        [dict_hyps_1], dict_hyps_1, "se", False
    )
    assert not package_target.check_hyps_convergence(
        [dict_hyps_2], dict_hyps_1, "se", False
    )
...
def test_get_next_best_acquisition():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(
            np.arange(0, 4), arr_acquisitions, cur_points
        )
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(arr_points, 1, cur_points)
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(
            arr_points, np.ones((4, 2)), cur_points
        )
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(arr_points, arr_acquisitions, 1)
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(
            arr_points, arr_acquisitions, np.arange(0, 3)
        )
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(
            arr_points, np.arange(0, 10), cur_points
        )
    with pytest.raises(AssertionError) as error:
        package_target.get_next_best_acquisition(
            arr_points, arr_acquisitions, np.ones((3, 5))
        )

    next_point = package_target.get_next_best_acquisition(
        arr_points, arr_acquisitions, cur_points
    )
