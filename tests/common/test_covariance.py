...
def test_choose_fun_grad_cov():
    ...
    assert (
        package_target.choose_fun_grad_cov("matern32")
        == package_target.grad_cov_matern32
    )
    assert (
        package_target.choose_fun_grad_cov("matern52")
        == package_target.grad_cov_matern52
    )
...
def test_get_kernel_inverse():
    ...
    truth_cov_X_X = np.array(
        [
            [1.00011000e00, 1.37095909e-06, 3.53262857e-24],
            [1.37095909e-06, 1.00011000e00, 1.37095909e-06],
            [3.53262857e-24, 1.37095909e-06, 1.00011000e00],
        ]
    )
    ...
    truth_grad_cov_X_X = np.array(
        [
            [
                [2.00002000e00, 0.00000000e00, 0.00000000e00, 0.00000000e00],
                [
                    2.74191817e-06,
...
def test_get_kernel_cholesky():
    ...
    truth_lower = [
        [1.00005500e00, 0.00000000e00, 0.00000000e00],
...
def test_cov_se():
    with pytest.raises(AssertionError) as error:
        package_target.cov_se(
            np.zeros((1, 2)), np.zeros((1, 2)), np.array([1.0, 1.0, 1.0]), 0.1
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_se(
            np.zeros((1, 2)), np.zeros((1, 3)), np.array([1.0, 1.0]), 0.1
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_se(
            np.zeros((1, 3)), np.zeros((1, 2)), np.array([1.0, 1.0]), 0.1
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_se(
...
def test_grad_cov_se():
    ...
    grad_cov_ = package_target.grad_cov_se(
        cov_, X_train, X_train, cur_hyps, num_hyps, False
    )
    print(grad_cov_)
...
def test_cov_matern32():
    ...
    cov_ = package_target.cov_matern32(
        X, Xp, cur_hyps["lengthscales"], cur_hyps["signal"]
    )
    print(cov_)
...
def test_grad_cov_matern32():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_matern32(
            "abc", X_train, X_train, cur_hyps, num_hyps, True
        )
    with pytest.raises(AssertionError) as error:
    ...
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_matern32(
            cov_, X_train, X_train, cur_hyps, num_hyps, "abc"
        )
    ...
    num_hyps = X_train.shape[1] + 1
    grad_cov_ = package_target.grad_cov_matern32(
        cov_, X_train, X_train, cur_hyps, num_hyps, True
    )
    print(grad_cov_)
...
def test_cov_matern52():
    ...
    cov_ = package_target.cov_matern52(
        X, Xp, cur_hyps["lengthscales"], cur_hyps["signal"]
    )
    print(cov_)
...
def test_cov_set():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.cov_set(
            str_cov,
            np.zeros((num_instances, num_dim + 1)),
            np.zeros((num_instances, num_dim)),
            np.array([1.0, 1.0, 1.0]),
            0.1,
        )
    ...
    cur_hyps = utils_covariance.get_hyps("matern52", 3)
    cov_ = package_target.cov_set(
        str_cov, bx, bxp, cur_hyps["lengthscales"], cur_hyps["signal"]
    )
    print(cov_)
...
def test_cov_main():
    cur_hyps = utils_covariance.get_hyps("se", 3)
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 2)), np.zeros((20, 3)), cur_hyps, False, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 3)), np.zeros((20, 2)), cur_hyps, False, jitter=0.001
        )
    with pytest.raises(ValueError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 2)), np.zeros((20, 2)), cur_hyps, False, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", 1.0, np.zeros((20, 3)), cur_hyps, False, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 2)), 1.0, cur_hyps, False, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            1.0, np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 3)), np.zeros((20, 3)), 2.1, False, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, "abc", jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 3)), np.zeros((12, 3)), cur_hyps, True, jitter=0.001
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False, jitter=1
        )
    with pytest.raises(AssertionError) as error:
        package_target.cov_main(
            "abc", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False, jitter=0.001
        )

    cur_hyps.pop("signal", None)
    with pytest.raises(ValueError) as error:
        package_target.cov_main(
            "se", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False
        )
    with pytest.raises(ValueError) as error:
        package_target.cov_main(
            "set_se", np.zeros((10, 5, 3)), np.zeros((20, 5, 3)), cur_hyps, False
        )

    cur_hyps = utils_covariance.get_hyps("se", 3)
    cov_ = package_target.cov_main(
        "se", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False, jitter=0.001
    )
    ...
    cur_hyps = utils_covariance.get_hyps("matern32", 3)
    cov_ = package_target.cov_main(
        "matern32", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False, jitter=0.001
    )
    assert np.all(cov_ == np.ones((10, 20)))
    ...
    cur_hyps = utils_covariance.get_hyps("matern52", 3)
    cov_ = package_target.cov_main(
        "matern52", np.zeros((10, 3)), np.zeros((20, 3)), cur_hyps, False, jitter=0.001
    )
    assert np.all(cov_ == np.ones((10, 20)))
...
def test_grad_cov_main():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            123, np.zeros((10, 2)), np.zeros((10, 2)), cur_hyps, True
        )
    with pytest.raises(AssertionError) as error:
    ...
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            "se", np.zeros((10, 2)), np.zeros((10, 2)), 123, True
        )
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            "se", np.zeros((10, 2)), np.zeros((10, 2)), cur_hyps, "abc"
        )
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            "abc", np.zeros((10, 2)), np.zeros((10, 2)), cur_hyps, True
        )
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            "se", np.zeros((10, 2)), np.zeros((10, 2)), cur_hyps, True, same_X_Xp="abc"
        )
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            "se", np.zeros((10, 2)), np.zeros((10, 2)), cur_hyps, True, same_X_Xp=False
        )
    with pytest.raises(AssertionError) as error:
        package_target.grad_cov_main(
            "se", np.zeros((10, 2)), np.zeros((10, 2)), cur_hyps, True, jitter="abc"
        )

    grad_cov_ = package_target.grad_cov_main(
        "se", np.ones((1, 2)), np.ones((1, 2)), cur_hyps, True
    )
    ...
    cur_hyps["lengthscales"] = 1.0
    grad_cov_ = package_target.grad_cov_main(
        "se", np.ones((1, 2)), np.zeros((1, 2)), cur_hyps, False
    )
