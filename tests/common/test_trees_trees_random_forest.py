...
def test_get_random_forest():
    ...
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, num_trees, depth_max, size_min_leaf, 1.23
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, num_trees, depth_max, size_min_leaf, "abc"
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, num_trees, depth_max, "abc", num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, num_trees, 1.23, size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, num_trees, "abc", size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, 12.0, depth_max, size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, Y, "abc", depth_max, size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, "abc", num_trees, depth_max, size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            X, np.array([1, 2, 3, 4]), num_trees, depth_max, size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            "abc", Y, num_trees, depth_max, size_min_leaf, num_features
        )
    with pytest.raises(AssertionError) as error:
        trees = package_target.get_random_forest(
            np.array([1, 2, 3, 4]), Y, num_trees, depth_max, size_min_leaf, num_features
        )
