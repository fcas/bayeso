...
def test_get_minimum():
    ...
    cur_minimum, cur_mean, cur_std, cur_sem = package_target.get_minimum(
        all_data, num_init
    )
    ...
    assert (
        cur_sem
        == np.std(truth_all_data, axis=0, ddof=1) / np.sqrt(truth_all_data.shape[0])
    ).all()
    ...
    assert (
        cur_sem == np.zeros(truth_all_data.shape[0]) / np.sqrt(truth_all_data.shape[0])
    ).all()
