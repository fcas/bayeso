...
def _cov_main_set(str_cov, num_dim, num_inst_X, num_inst_Xs, num_set_X, num_set_Xs):
    ...
    cov_ = covariance.cov_main(
        str_cov,
        np.zeros((num_set_X, num_inst_X, num_dim)),
        np.zeros((num_set_Xs, num_inst_Xs, num_dim)),
        cur_hyps,
        False,
        jitter=0.001,
    )
...
def test_cov_se(benchmark):
    cov_ = benchmark.pedantic(
        _cov_main_vector, args=("se", 3, 1000, 2000), rounds=num_rounds, iterations=1
    )
    print(cov_)
...
def test_cov_set_matern32(benchmark):
    cov_ = benchmark.pedantic(
        _cov_main_set,
        args=("set_matern32", 3, 100, 200, 10, 40),
        rounds=num_rounds,
        iterations=1,
    )
    print(cov_)
...
def test_cov_set_matern52(benchmark):
    cov_ = benchmark.pedantic(
        _cov_main_set,
        args=("set_matern52", 3, 100, 200, 10, 40),
        rounds=num_rounds,
        iterations=1,
    )
    print(cov_)
