...
def get_optimized_kernel(
    ...
    if str_optimizer_method in ["L-BFGS-B", "SLSQP"]:
        bounds = utils_covariance.get_range_hyps(
            str_cov, num_dim, use_ard=use_ard, fix_noise=fix_noise, use_gp=False
        )
    ...
    hyps = utils_covariance.restore_hyps(
        str_cov, result_optimized, use_ard=use_ard, fix_noise=fix_noise, use_gp=False
    )
