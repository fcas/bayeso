...
@utils_common.validate_types
def _save_figure(
    path_save: str, str_postfix: str, str_prefix: str = ""
) -> constants.TYPE_NONE:  # pragma: no cover
    """
    It saves a figure.
    ...
    if path_save is not None and str_postfix is not None:
        ...
        plt.savefig(
            os.path.join(path_save, str_figure + ".pdf"),
            format="pdf",
            transparent=True,
            bbox_inches="tight",
        )
...
@utils_common.validate_types
def _show_figure(
    pause_figure: bool, time_pause: constants.TYPING_UNION_INT_FLOAT
) -> constants.TYPE_NONE:  # pragma: no cover
    """
    It shows a figure.
...
def plot_gp_via_distribution(
    ...
    if Y_test is not None:
        ax.plot(
            X_test.flatten(), Y_test.flatten(), c=colors[1], linewidth=4, marker="None"
        )
    ax.plot(
        X_test.flatten(), mean_test.flatten(), c=colors[2], linewidth=4, marker="None"
    )
    ax.fill_between(
        X_test.flatten(),
    ...
    ax.plot(
        X_train.flatten(), Y_train.flatten(), "x", c=colors[0], markersize=10, mew=4
    )
    ...
    _set_ax_config(
        ...
        str_x_axis,
        str_y_axis,
        xlim_min=np.min(X_test),
        xlim_max=np.max(X_test),
        draw_zero_axis=draw_zero_axis,
    )
...
@utils_common.validate_types
def plot_minimum_vs_iter(
    minima: np.ndarray,
    list_str_label: constants.TYPING_LIST[str],
    num_init: int,
    draw_std: bool,
    ...
    _set_ax_config(
        ax, str_x_axis, str_y_axis, xlim_min=0, xlim_max=mean_min.shape[0] - 1
    )
    ...
    if path_save is not None and str_postfix is not None:
        ...
        fig_legend.savefig(
            os.path.join(path_save, f"legend_{str_postfix}.pdf"),
            format="pdf",
            transparent=True,
            bbox_inches="tight",
        )
...
def plot_minimum_vs_time(
    ...
    assert (
        ...
        or times.shape[2] + num_init == minima.shape[2]
    )
    ...
    for ind_minimum, arr_minimum in enumerate(minima):
        ...
        _, mean_min, std_min, _ = utils_common.get_minimum(arr_minimum, num_init)
        x_data = utils_common.get_time(
            times[ind_minimum], num_init, times.shape[2] == minima.shape[2]
        )
        y_data = mean_min
        std_data = std_min
...
def plot_bo_step(
    ...
    if num_init is not None:
        ...
        ax.plot(X_train[:-1, :], Y_train[:-1, :], "rx", ms=14, markeredgewidth=6)
        ax.plot(
            X_train[-1, :],
            Y_train[-1, :],
            c="orange",
...
def plot_bo_step_with_acq(
    ...
    if num_init is not None:
        ...
        ax1.plot(X_train[:-1, :], Y_train[:-1, :], "rx", ms=14, markeredgewidth=6)
        ax1.plot(
            X_train[-1, :],
            Y_train[-1, :],
            c="orange",
