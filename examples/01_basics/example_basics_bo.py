...
model_bo = bo.BO(np.array([[-6.0, 6.0]]), normalize_Y=False)
...
for ind_ in range(1, num_iter + 1):
    ...
    next_x, dict_info = model_bo.optimize(
        X_train, fun_target(X_train), str_sampling_method="sobol", seed=ind_
    )
    ...
    str_postfix = f"basics_bo_{ind_}" if path_save is not None else None
